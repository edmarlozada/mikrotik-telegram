...
/system script add name=\$tName source=\$tSave owner=telegram comment=\"telegram_savedata: \$tGrp\";
...
# if sending failed save to outbox and call eTGHandler
:if (\$iCtr=0) do={
...
# Author: Chloe Renae & Edmar Lozada\r
# ------------------------------\r
:if ([/system script find owner=telegram]!=\"\") do={
  :local gTGList [:toarray \"\"];
  :foreach nRec in=[/system script find owner=telegram] do={ :set (\$gTGList->[:len \$gTGList]) \$nRec };
